...
namespace TwitterClient
{
    public class Twitter
    {
        ...
        public string GetTimeline(int count)
        {
            string url;
            string queryString;
            Uri requestUri = new Uri("http://twitter.com/statuses/home_timeline.xml?count=" + count);
        ...
        public void PostTweet(string tweet)
        {
            string url;
            string queryString;
            Uri requestUri = new Uri("http://twitter.com/statuses/update.xml?status=" + oauthbase.UrlEncode(tweet, Encoding.UTF8));
            string signature = oauthbase.GenerateSignature(requestUri, token.consumerKey, token.consumerSecret, token.oAuthToken, token.oAuthTokenSecret,
